...
class RecommendationiEngine():
    ...
    def __init__(self):
        ...
        self.books = self.books.rename({'id':'book_id', 'title':'book_title', 'image':'book_image'}, axis=1)

        self.books_category = pd.read_sql_query("SELECT `id` as `category_id`, `title` as `category_title`, `image` as `category_image` FROM `libraryapp_bookcategory`", conn)

        # Merge book_list and book_category
        self.books_data = pd.merge(self.books, self.books_category, on='category_id')

        self.ratings = pd.read_sql_query("SELECT `rating`, `book_id`, `user_id` FROM `libraryapp_bookrating`",conn)
        # print(ratings.columns)

        self.books_with_rating = pd.merge(self.books_data, self.ratings, on='book_id')

        self.books_mat = self.books_with_rating.pivot_table(index='user_id', columns='book_title', values='rating')
    ...
    def get_recommendation(self, user_id, corr_limit):
        books_list = self.books_with_rating.loc[self.books_with_rating['user_id'] == user_id]['book_title']
